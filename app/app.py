...
# Dashboard ATP
    ...
    st.caption("Sección corregida: aquí solo existe UN box plot. El resto son tarjetas, barras, líneas y heatmap para evitar redundancia visual.")
    ...
    with c1:
        ...
        surf = matches.dropna(subset=["year", "Surface"]).copy()
        surf["year"] = surf["year"].astype(int)
        surf = surf.groupby(["year", "Surface"], as_index=False).agg(partidos=("match_id", "nunique"))
    ...
    st.subheader("Único box plot del Dashboard: % puntos ganados con primer servicio")
    st.caption("Esta es la única gráfica tipo box plot del Dashboard. La caja representa la distribución del Top 100; el punto rojo sitúa a Carlos Alcaraz.")
...
# Carlos Alcaraz
    ...
    st.markdown("<div class='alcaraz-note'>El bloque superior resume sus records antes de entrar en las gráficas. El rojo se reserva para identificar a Alcaraz cuando aparece contra otros jugadores; en las gráficas internas se usan colores por superficie o por métrica.</div>", unsafe_allow_html=True)
    ...
    st.subheader("Único box plot de Alcaraz: Winner/UE vs Top 100")
...
# Comparador ATP
    ...
    st.subheader("Único box plot del comparador: Winner/UE Ratio")
